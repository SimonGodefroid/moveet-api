...
var jsonNowShowing = './save/tmp/moviesNowShowing.json';
		...
		for (var i = 1; i <= request_numbers; i++) {
			request(
				...
				function(error, response, body) {
					...
					if (completed_requests === request_numbers) {
						console.log(jsonNowShowing);
						console.log(allResults.length);
						var dataToSave = [].concat.apply([], allResults);
						jsonfile.writeFile(jsonNowShowing, dataToSave, function(err) {
							if (err !== null) {
								console.error(err);
							} else {
								console.log('JSON saved with ' + allResults.length * 25 + ' nowshowing movies');
							}
						});
					}
				}
