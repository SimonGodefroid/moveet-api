...
var jsonComingSoon = './save/tmp/moviesComingSoon.json';
			...
			request(
				...
				function(error, response, body) {
					...
					if (completed_requests === request_numbers) {
						console.log(jsonComingSoon);
						console.log(allResults.length);
						var dataToSave = [].concat.apply([], allResults);
						jsonfile.writeFile(jsonComingSoon, dataToSave, function(err) {
							if (err !== null) {
								console.error(err);
							} else {
								console.log('JSON saved with ' + allResults.length * 25 + ' comingsoon movies');
							}
						});
						jsonfile.writeFile(jsonFailedRequestsSaveComingSoon, failedRequestsSaveComingSoon, function(
							err
						) {
							if (err !== null) {
								console.error(err);
